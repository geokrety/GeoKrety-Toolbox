...
// @description	(v0.3.6) Shows GeoKrety trackables on geocaching.com cache pages and facilitates dropping GK trackables into GC caches.
...
// HISTORY
// -------
// 0.3.6     25 Mar 2019     FIx document.evaluate() parameters (Issue: #3)
...
window.gk_getinventory = function() {
	...
	xhr.onload  = function() {
			...
			else
			{
				...
				{
					gk_inventory.innerHTML = '<span style="line-height:1.2em;">Your are not logged in on GeoKrety.org. To view your inventory you need to <a href="//'+GKHOST+'/longin.php">log in</a> and then click refresh inventory.</span>';
				}
			}
	}
...
function go() {
	...
	tmp += '<tr><td colspan=2 style="padding:0;line-height:1px">'+pixel_grey+'</td></tr><tr><td style="padding:0;width=16px"><center>'+imgLogGK+'</center></td><td><a id="gk_logkret" href="//'+GKHOST+'/ruchy.php?gkt=log_gc&waypoint='+gcwaypoint+'&lat='+lat+'&lon='+lon+'">Log any GeoKret here</a></td></tr>';
...
window.gk_searchgeokrets = function() {
	...
	var xhr = new XMLHttpRequest();
	var url = "//"+GKHOST+"/gkt/search_v3.php?mode=latlon&lat="+lat+"&lon="+lon;
	xhr.onerror = function() {
		/*debug('Request failed with ' + xhr.status + ' ' + xhr.statusText + '\nData:\n' + xhr.responseText);*/
	...
	xhr.onload  = function() {
		...
		if (gk.length>0)
			for (var i=gk.length-1; i>=0; i--) {
				krets='<tr><td style="width:16px;padding:2px">'+imgGK+'</td><td><a href="//'+GKHOST+'/konkret.php?id='+gk[i].id+'" target="_blank">' + gk[i].n + '</a></td></tr>'+krets;
			}
...
function init() {
	...
	debug('Toolbox starting!');

	var tmp, url, cond1, cond2;
	url = window.location.href;

	debug('Checking URL...');
	regex = /https?:\/\/www\.geocaching\.com\/geocache\/GC/;
	if (url.match(regex)) {
		debug('Match found');
		...
		if (document.getElementById('ctl00_ContentBody_CoordInfoLinkControl1_uxCoordInfoCode').textContent.match(/(GC[A-Z0-9]+)/)) { gcwaypoint=RegExp.$1; cond1=true; debug('Waypoint found'); }
			else { gcwaypoint=''; cond1=false; debug('Waypoint not found');}

		debug('Searching for coordinates...');
		if (document.evaluate("//script[contains(., 'MapTilesEnvironment')]", document).iterateNext().textContent.match(/oldLatLngDisplay":"([NS])\s(\d{1,3}).\s(\d{1,2})\.(\d{3}).\s([EW])\s(\d{1,3}).\s(\d{1,2})\.(\d{3})."/)) { cond2=true; debug('Coordinates found'); }
